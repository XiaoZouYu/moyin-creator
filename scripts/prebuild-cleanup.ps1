...
foreach ($process in Get-Process -Name "santi-creator", "三体漫创" -ErrorAction SilentlyContinue) {
...
foreach ($process in Get-CimInstance Win32_Process -ErrorAction SilentlyContinue) {
  ...
  if (
    ...
    (($name -and $name -like "santi-creator-*-setup.exe") -and (Test-IsPathUnderDirectory -Path $exePath -Directory $normalizedReleaseDir))
  ) {
