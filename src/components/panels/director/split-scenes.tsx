...
export function SplitScenes({ onBack, onGenerateVideos }: SplitScenesProps) {
  ...
  const processReferenceImagesForApi = useCallback(async (
    ...
    return prepareImageReferencesForApi(referenceImages, {
  ...
  const handleGenerateSingleVideo = useCallback(async (sceneId: number) => {
    ...
    try {
      ...
      const firstFrameConverted = await resolveImageToHttpUrl(normalizedFirstFrame, {
      ...
      if (lastFrameUrl) {
        ...
        const lastFrameConverted = await resolveImageToHttpUrl(normalizedLastFrame, {
